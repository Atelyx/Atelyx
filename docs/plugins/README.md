...
| 面 | 说明 |
| --- | --- |
| `ctx.<service>` | 类型化服务：`state`/`app`/`shell`/`vault`/`dialog`/`clipboard`/`window`/`ai`/`collab`/`canvas`/`table`（见 [ctx API](ctx-api.md)） |
| `ctx.events` | 领域事件总线：`ctx.events.on("canvas:changed", ...)` 订阅（`table:changed`/`vault:changed` 等，见 [ctx API](ctx-api.md)） |
| `ctx.slots` | UI 注册：`ctx.slots.registerView(...)`（工作区面板视图）/ `ctx.slots.registerTableView(...)`（表格编辑器内视图） |
...
`theme`/`tool`/`command`/`background`）；实际能力全部在 `apply` 运行时注册（视图经 `ctx.slots`，
逻辑经 `ctx` 服务/事件）。

## 快速开始

1. 按[清单格式](manifest.md)写 `package.json` + 入口源码。
2. 在 `apply` 里注册能力：`ctx.slots.registerView(...)`、`ctx.on(...)`、调用 `ctx.<service>`。
