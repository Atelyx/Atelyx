...
参数全开即等价任意命令执行。选项里 `cwd` 指定工作目录；`env` 是**追加/覆盖**宿主环境（不传就完全
继承宿主的 `PATH`/`TEMP` 等，本机服务通常需要）；不提供 stdin 写入。

插件启动的进程**按插件记账**：插件停用、卸载、更新、回退以及宿主重载插件时都会结束它们——
长驻服务不该活过插件运行时（启动还没返回时被停用也一样会被结束）。**应用退出时同样一并结束**：
进程在创建那一刻就被纳入清理范围，所以不要用它去甩出一个「应在应用关闭后继续跑」的进程。
插件因此不必自己登记清理，但仍应把停止入口（设置页的按钮等）做给用户；`cancel()` 之后再停用
不会有副作用（已结束的句柄为 no-op）。注意「进程随运行时结束」也意味着服务不能跨重载存活：
要复用已在跑的服务，请自查端口/接口后再决定是否启动（插件被重载时上一轮的 `{ pid, cancel }`
句柄已随作用域失效）。托管进程需要活过切换仓库的场景时，在清单声明
`keepMountedOnVaultSwitch`（见「切仓库保活」）——声明后切仓库重载不再结束进程，其余重载照常。
