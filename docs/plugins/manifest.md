...
- `main` 指向入口文件（`.js`/`.ts`/`.tsx`），默认导出 `apply(ctx, config)`。
- TS 入口发布源码即可，宿主内置转译器加载时转译；入口须自包含（无运行时 import）。
- 纯 theme 插件（`type: "theme"` 且无其他代码类型）可省略 `main`——主题是声明式皮肤。

## 披露 `declares` 与完全自由模型

**完全自由模型：无运行时拒绝**——`declares` 是诚实披露（管理页与审计对照展示），安全责任在
用户知情（安装警告）。宿主服务里的敏感项（如 `shell`/`clipboard`）在管理页以「敏感」高亮。
...
- `themes`：非空数组；`id` 插件内唯一；`colorScheme` 为 `light`/`dark`（固定基底，不跟随系统；
  ...
  **只覆盖想改的子集**，未覆盖的落回内置浅/深基础方案。与内置基底 `light`/`dark` 重名的条目会被拒绝。
