 ...
 * 断言上一个仓库的 per-file 状态全部为空：笔记内容缓存 / 挂起输入 / 保存状态 / 外部修改序号、
 * 笔记撤销栈、协作文档绑定（Y.Doc 上下文）、编辑会话、画布运行时、表格运行时与图片显示缓存、
 * 画布视口交接缓存。任何一类残留都会让新仓库同路径文件串到旧仓库内容（陈旧正文 / 陈旧图片）。
 *
...
async function seedAllDomains(): Promise<void> {
  ...
  canvasStore.useCanvasStore.setState({ canvasFile: "a.atlx", dirty: true });
  // 表格运行时
