...
const h = vi.hoisted(() => {
  ...
  return {
    ...
    /** 挂起下一次 vault_config_patch（写盘在途模拟）。 */
    gatePatch() {
  ...
  invoke: async (cmd: string, args?: Record<string, unknown>) => {
    ...
    switch (cmd) {
      case "read_vault_config":
        return { config: h.state.vaultConfig, corruptBackup: h.state.corruptBackup };
      ...
      case "read_folder_colors":
        return {};
      case "get_api_key":
        if (h.state.failGetKey) throw new Error("keychain 不可用");
        return h.state.keychain.get(h.keyOf(a)) ?? "";
      case "set_api_key":
        if (h.state.failSetKey) throw new Error("keychain 不可用");
        h.state.setKeyCalls.push(h.keyOf(a));
        h.state.keychain.set(h.keyOf(a), String(a.key));
        return null;
      case "delete_api_key":
        h.state.keychain.delete(h.keyOf(a));
        return null;
      default:
        return null;
    }
...
describe("syncKeys 关闭时的明文 key 边界", () => {
  it("persist 写盘路径（改供应商）顺手抹掉磁盘残留的明文 key", async () => {
    ...
    const providers = h.state.patches[0].providers as Array<Record<string, unknown>>;
    expect(providers).toHaveLength(1);
    expect(providers[0].apiKey).toBeUndefined();
    // 残留明文 key 不在补丁里重写，而是显式删键（合并语义下省略 = 保留磁盘现值）
    expect(searchOf(h.state.patches[0]).tavilyApiKey).toBeNull();
    const merged = mergeInto(h.state.vaultConfig, h.state.patches[0]);
  ...
  it("残留 key 只删一次：写盘后再改字段的补丁不再携带删键指令", async () => {
    ...
    expect(Object.keys(h.state.patches[1])).toEqual(["fileExplorerSort"]);
  });

  it("改排序只发排序一个字段（磁盘无残留 key 时其余字段由 Rust 侧按磁盘合并保留）", async () => {
    h.state.vaultConfig = { vaultRoot: "v1", search: { provider: "tavily" } };
    await settings.useSettingsStore.getState().loadVaultConfig();
    await settings.useSettingsStore.getState().setFileExplorerSort("name-asc");

    expect(h.state.patches).toHaveLength(1);
    expect(h.state.patches[0]).toEqual({ fileExplorerSort: "name-asc" });
  });

  it("改 SearXNG 地址不会把 config.json 里残留的明文 key 写回（并显式删键）", async () => {
    h.state.vaultConfig = { vaultRoot: "v1", search: { provider: "tavily", tavilyApiKey: "tvly-secret" } };
    await settings.useSettingsStore.getState().loadVaultConfig();
    ...
    expect(h.state.patches).toHaveLength(1);
    const search = searchOf(h.state.patches[0]);
  ...
  it("syncKeys 开启时 key 随仓库落盘（不得误剥离）", async () => {
    ...
    expect(searchOf(h.state.patches[0]).tavilyApiKey).toBe("tvly-secret");
  });

  it("关闭时残留 key 采纳进本机 keychain（不静默丢弃）", async () => {
    h.state.vaultConfig = { vaultRoot: "v1", search: { provider: "tavily", tavilyApiKey: "tvly-stray" } };
    await settings.useSettingsStore.getState().loadVaultConfig();

    expect(h.state.setKeyCalls).toEqual(["v1:search-tavily"]);
    expect(h.state.keychain.get("v1:search-tavily")).toBe("tvly-stray");
    expect(settings.useSettingsStore.getState().tavilyKey).toBe("tvly-stray");
  });

  it("keychain 读失败时仍能采纳残留 key，且不阻断加载", async () => {
    h.state.failGetKey = true;
    h.state.vaultConfig = { vaultRoot: "v1", search: { provider: "tavily", tavilyApiKey: "tvly-stray" } };
  ...
  it("关闭 syncKeys 开关时写盘剥离 key 并回写 keychain", async () => {
    ...
    expect(h.state.patches[0].syncKeys).toBe(false);
    // 关键：清空内层 key 只能靠显式 null（合并语义下「省略键」= 保留磁盘现值），
    // 发 undefined 会被 JSON 丢弃 → 文件里的明文 key 永远留着
    expect(searchOf(h.state.patches[0]).tavilyApiKey).toBeNull();
    expect(h.state.keychain.get("v1:search-tavily")).toBe("tvly-secret");
  });
  ...
  it("合并补丁语义：显式 null 删键、省略键保留磁盘现值", async () => {
    ...
    const disk = mergeInto(h.state.vaultConfig, h.state.patches[0]);
  ...
  it("本机 keychain 已有 key 时不覆盖", async () => {
    h.state.keychain.set("v1:search-tavily", "tvly-local");
    h.state.vaultConfig = { vaultRoot: "v1", search: { provider: "tavily", tavilyApiKey: "tvly-stray" } };
    await settings.useSettingsStore.getState().loadVaultConfig();

    expect(h.state.setKeyCalls).toEqual([]);
...
describe("脏门控与字段级补丁（撕裂窗口覆盖防护）", () => {
  it("无变化时 flush 不写盘", async () => {
    h.state.vaultConfig = { vaultRoot: "v1", providers: [{ id: "p1", name: "A", baseUrl: "u", models: [] }] };
    await settings.useSettingsStore.getState().loadVaultConfig();
  ...
  it("改供应商只发 providers，不携带陈旧副本的其它字段", async () => {
    ...
    expect(h.state.patches).toHaveLength(1);
    expect(Object.keys(h.state.patches[0])).toEqual(["providers"]);
  });

  it("写盘在途期间切换仓库：providers 不回填进新仓库的设置态、基线不推进", async () => {
    h.state.vaultConfig = {
      vaultRoot: "v1",
      providers: [{ id: "p1", name: "A", baseUrl: "u", models: [] }],
    };
    ...
    h.gatePatch();
    // removeProvider 直连 persist（不经 debounce）：删空 providers
    const pending = settings.useSettingsStore.getState().removeProvider("p1");
    await h.waitGateArmed();
    // 写盘在途期间切到 v2
    const app = await import("./appStore");
    app.useAppStore.setState({
      vaultRoot: "v2",
      vaultIdentity: { kind: "local", root: "v2" },
    });
    h.releaseGate();
    await pending;

    // 补丁本身已按捕获身份写进 v1（写盘前校验通过）
    expect(h.state.patches).toHaveLength(1);
    // 但内存回填被身份复检丢弃：vaultConfig.providers 保持磁盘加载时的 v1 快照，不被 persist 的空数组覆盖
    const providers = settings.useSettingsStore.getState().vaultConfig?.providers;
    expect(providers).toHaveLength(1);
  });
});
...
describe("配置损坏的可见性", () => {
  it("后端报告损坏备份时弹出通知（用户要知道设置与 key 为何消失）", async () => {
    ...
    h.state.vaultConfig = {};
    h.state.corruptBackup = "config.json.corrupt-abc";
    await settings.useSettingsStore.getState().loadVaultConfig();
  ...
  it("正常读取不弹通知", async () => {
    const notifications = await import("./notificationStore");
    h.state.vaultConfig = { vaultRoot: "v1" };
    await settings.useSettingsStore.getState().loadVaultConfig();
  ...
  it("写盘路径发现原文损坏（后端已备份）也弹通知", async () => {
    ...
    h.state.vaultConfig = { vaultRoot: "v1" };
    await settings.useSettingsStore.getState().loadVaultConfig();
    h.state.patchCorruptBackup = "config.json.corrupt-write";
