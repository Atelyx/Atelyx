...
- **局域网协作** — 多人实时互见：同编一篇笔记、同改一块画布、同看一张表格。谁在看什么、选中在哪里，成员实时可见。
- **文件即仓库** — 没有数据库。画布、笔记、附件都是普通文件，可积累、可备份、可 Git 同步，也可被外部编辑器打开并实时同步回来。
...
插件是一个 Git 仓库：在 `package.json` 的 `atelyx` 块里声明入口、能力面与作用域，推送到 GitHub 并打上 `atelyx-plugin` 主题，即被插件市场自动收录。开发指南见 [插件开发文档](docs/plugins/README.md)。
